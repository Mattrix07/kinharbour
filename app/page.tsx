...
const questions = [
  { section: "Family context", eyebrow: "Care recipient", title: "Who is this care plan for?", helper: "Use their first name if you are comfortable.", type: "text", placeholder: "e.g. Margaret, Dad, Mum" },
  { section: "Family context", eyebrow: "Location", title: "Where should care be centred?", helper: "Add where the person needing care lives and where close family are based.", type: "address" },
  ...
  { section: "Family context", eyebrow: "Trigger", title: "What is prompting this decision now?", helper: "Select the closest reason.", type: "single", options: ["Recent fall", "Hospital discharge", "Memory or confusion concerns", "Family can no longer manage", "Planning ahead", "Not sure — we just need guidance"] },
  { section: "Family context", eyebrow: "Living situation", title: "What best describes their current living situation?", helper: "This helps assess safety and informal support.", type: "single", options: ["Living alone", "Living with partner", "Living with family", "Currently in hospital", "Currently in respite", "Not sure"] },
  { section: "Care needs", eyebrow: "Support needs", title: "What kind of support is needed?", helper: "Select all that apply.", type: "multi", options: ["Showering or personal care", "Meals and groceries", "Medication reminders", "Nursing or wound care", "Transport", "Cleaning and home help", "Memory support", "Overnight supervision"] },
  { section: "Care needs", eyebrow: "Mobility", title: "How would you describe their mobility?", helper: "Mobility is a strong indicator of care intensity.", type: "single", options: ["Independent", "Uses a walking stick", "Uses a walker", "Needs help transferring", "Wheelchair or mostly bed-bound", "Not sure"] },
  { section: "Care needs", eyebrow: "Cognition", title: "Are there memory, confusion or dementia-related concerns?", helper: "This helps decide whether memory support may be relevant.", type: "single", options: ["No major concerns", "Some forgetfulness", "Frequent confusion", "Diagnosed dementia", "Wandering or safety concerns", "Not sure"] },
  ...
  { section: "Pathway", eyebrow: "Urgency", title: "How quickly does a decision need to be made?", helper: "Urgency changes the recommended next steps.", type: "single", options: ["Planning ahead", "Within 1–3 months", "This month", "This week", "Next 72 hours", "Not sure"] },
  { section: "Pathway", eyebrow: "Preferred setting", title: "Which care setting are you currently considering?", helper: "It is okay to be unsure.", type: "single", options: ["Home care package", "Short-term respite", "Residential aged care", "Hospital discharge / transition care", "We want to keep them at home", "Not sure"] },
  { section: "Pathway", eyebrow: "Funding status", title: "What is their current funding or My Aged Care status?", helper: "Many families are unsure at this stage.", type: "single", options: ["Not registered with My Aged Care", "Registered but no assessment yet", "ACAT assessment booked", "Home Care Package approved", "Residential care approved", "I don’t know / not sure"] },
  { section: "Pathway", eyebrow: "Priorities", title: "What matters most in the care decision?", helper: "Select the priorities the family will care about most.", type: "multi", options: ["Staying close to home", "Keeping costs manageable", "Dementia or memory support", "Clinical care quality", "Availability now", "Family visibility", "Warmth and dignity"] },
];
  ...
  return (
    <div className="relative origin-center scale-[0.9] lg:origin-left">
      <div className="mb-5 flex flex-wrap justify-center gap-3 lg:justify-start">
        {Object.keys(publicModes).map((item) => <button key={item} onClick={() => setMode(item)} className={cx("rounded-full border px-4 py-2 text-sm font-medium", mode === item ? "border-[#0b2c34] bg-[#0b2c34] text-white" : "border-[#d9dfda] bg-white/72 text-[#53666a]")}>{item}</button>)}
      </div>
  ...
  return (
    <main>
      <section className="mx-auto grid min-h-[calc(100vh-76px)] max-w-7xl gap-8 px-6 pb-24 pt-12 lg:grid-cols-[1.04fr_.96fr] lg:items-center">
        <div><div className="mb-7 inline-flex items-center gap-3 rounded-full border border-[#cfe0d8] bg-[#edf5f2]/75 px-4 py-2 text-sm font-semibold text-[#4d8d8b] shadow-sm"><Sparkles className="h-4 w-4" />Built for Australian families</div><h1 className="max-w-4xl text-[3.8rem] font-semibold leading-[.92] tracking-[-.065em] md:text-[5.65rem]">Find the right aged care path, <span className="text-[#5d9691]">step by step.</span></h1><p className="mt-7 max-w-2xl text-lg leading-8 text-[#62756e]">KinHarbour brings aged care guidance, likely costs, a shared family planner and personalised recommendations into one all-in-one place.</p><div className="mt-8 flex flex-col gap-3 sm:flex-row"><button onClick={() => setActive("assessment")} className="inline-flex items-center justify-center gap-2 rounded-full bg-[#0b2c34] px-6 py-3.5 text-sm font-semibold text-white shadow-xl">Begin Assessment <ArrowRight className="h-4 w-4" /></button><button className="inline-flex items-center justify-center rounded-full border border-[#0b2c34] px-6 py-3.5 text-sm font-semibold text-[#0b2c34]">Book a Strategy Call</button></div><div className="mt-9 flex flex-wrap gap-x-7 gap-y-3 text-sm font-medium text-[#7c8c87]">{["Independent guidance", "Australian-built", "Designed for overwhelmed families, not experts"].map((item) => <div key={item} className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-[#5d9691]" />{item}</div>)}</div></div>
        <DashboardPreview />
      </section>
      <section className="mx-auto max-w-7xl px-6 pb-28 pt-4"><div className="rounded-[2rem] border border-white/70 bg-white/45 p-8 shadow-sm backdrop-blur-xl"><div className="mb-9 text-center"><p className="text-sm font-bold uppercase tracking-[.22em] text-[#89a697]">What we do</p><h2 className="mt-3 text-4xl font-semibold tracking-[-.045em] md:text-5xl">Simple, personal, shareable guidance.</h2></div><div className="grid gap-7 md:grid-cols-3">{whatWeDo.map((item, index) => { const Icon = item.icon; return <div key={item.title} className="relative rounded-[1.7rem] border border-[#dfe7e2] bg-white/80 p-6 shadow-sm"><div className="absolute -top-4 left-1/2 grid h-8 w-8 -translate-x-1/2 place-items-center rounded-full border border-[#dfe7e2] bg-white text-xs font-bold text-[#53666a]">{index + 1}</div><div className="flex items-start gap-4"><span className="grid h-14 w-14 shrink-0 place-items-center rounded-2xl bg-[#e5efec]"><Icon className="h-6 w-6 text-[#5d9691]" /></span><div><h3 className="text-xl font-semibold tracking-tight">{item.title}</h3><p className="mt-2 leading-7 text-[#62756e]">{item.text}</p></div></div></div>; })}</div></div></section>
      <section className="mx-auto max-w-7xl px-6 pb-24"><div className="rounded-[2rem] bg-[#eef6f3]/80 p-8 shadow-sm"><div className="grid gap-8 lg:grid-cols-[.9fr_2.1fr] lg:items-center"><div><h2 className="text-4xl font-semibold leading-[1] tracking-[-.05em]">Trusted by Australian families</h2><p className="mt-4 leading-7 text-[#62756e]">Designed for elderly Australians and the families who care for them.</p><div className="mt-6 flex items-end gap-3"><span className="text-4xl font-semibold">4.9</span><span className="pb-1 text-lg tracking-[.18em] text-[#b88745]">★★★★★</span></div></div><div className="grid gap-5 md:grid-cols-3">{reviews.map((review) => <div key={review.name} className="rounded-[1.6rem] border border-white/70 bg-white/86 p-6 shadow-sm"><p className="min-h-[108px] text-[15px] italic leading-7 text-[#43565c]">“{review.quote}”</p><div className="mt-5 border-t border-[#e7ede9] pt-4"><p className="font-semibold">{review.name}</p><p className="text-sm text-[#62756e]">{review.role}</p></div></div>)}</div></div></div></section>
    </main>
  );
}

function WhyPage() {
  return <main className="mx-auto max-w-7xl px-6 py-14"><div className="mb-6 inline-flex items-center gap-3 rounded-full border border-[#cfe0d8] bg-[#edf5f2]/75 px-4 py-2 text-sm font-semibold text-[#4d8d8b]"><Sparkles className="h-4 w-4" />Why KinHarbour</div><h1 className="max-w-5xl text-6xl font-semibold leading-[.95] tracking-[-.07em] md:text-7xl">When care becomes urgent, most families have <span className="text-[#5d9691]">no clear starting point.</span></h1><p className="mt-7 max-w-3xl text-xl leading-9 text-[#62756e]">The aged care system is complex, fragmented, and overwhelming — especially when decisions need to happen fast.</p><div className="mt-10 grid gap-5 md:grid-cols-3">{[["2,700+", "aged care homes and services to compare"], ["95,000", "people waiting for home support in late 2025"], ["83,000", "new at-home care packages planned over two years"]].map(([value, label]) => <Card key={value}><p className="text-4xl font-semibold text-[#0b2c34]">{value}</p><p className="mt-3 leading-7 text-[#53666a]">{label}</p></Card>)}</div></main>;
}
...
function AssessmentPage({ setActive, setProfile }) {
  ...
  const sectionRanges = [{ label: "Family context", to: 4 }, { label: "Care needs", to: 8 }, { label: "Pathway", to: 12 }];
