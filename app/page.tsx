...
const fallbackCarePlan = {
  id: "fallback-care-plan",
  generatedBy: "fallback",
  profile: {
    careName: "Margaret",
    assessorRole: "Daughter",
    recipientAddress: "Bondi Junction, NSW",
    familyAddress: "Sydney, NSW",
  },
  recommendedPathway: "Home care services",
  pathwayType: "home_support",
  currentStage: "Home Care Assessment",
  confidence: 87,
  ...
  summary: "Margaret is currently best aligned to home care services. Home care services now, with residential planning on watch.",
  actionItems: [
    "Register or confirm status with My Aged Care",
    "Book or review ACAT assessment",
    "Compare recommended home care providers near Bondi Junction, NSW",
  ...
  costEstimate: {
    fundingMode: "public/package-funded with possible co-contribution",
    weeklyEstimate: 790,
    annualEstimate: 41080,
    governmentWeekly: 569,
    outOfPocketWeekly: 221,
    assumptions: [
  ...
  sources: [
    { title: "My Aged Care — Apply for an assessment", url: "https://www.myagedcare.gov.au/assessment/apply-online", relevance: "Assessment pathway" },
    { title: "My Aged Care — Find a provider", url: "https://www.myagedcare.gov.au/find-a-provider", relevance: "Provider comparison" },
    { title: "Department of Health and Aged Care — Support at Home", url: "https://www.health.gov.au/our-work/support-at-home", relevance: "Home care funding context" },
  ],
};
...
function AssessmentPage({ setActive, setProfile, setCarePlan, setSavedIds, setCompleted }) {
  ...
  function answer(option) {
    setAnswers((current) => {
      if (q.type === "multi") {
        const existing = Array.isArray(current[step]) ? current[step] : [];
        return { ...current, [step]: existing.includes(option) ? existing.filter((x) => x !== option) : [...existing, option] };
      }
  ...
  async function finish() {
    ...
    const profile = {
      careName: typeof answers[0] === "string" && answers[0] ? answers[0] : "Margaret",
      recipientAddress: location.recipient || "Bondi Junction, NSW",
      familyAddress: location.family || "Sydney, NSW",
      assessorRole: typeof answers[2] === "string" ? answers[2] : "Daughter",
    };
    ...
    try {
      ...
      const plan = data.carePlan || fallbackCarePlan;
      setCarePlan(plan);
      setProfile(plan.profile || profile);
      setSavedIds((plan.matchedProviders || []).slice(0, 2).map((provider) => provider.id));
      setCompleted([0, 1]);
      setActive("preview");
    } catch {
      setProfile(profile);
      setCarePlan({ ...fallbackCarePlan, profile });
      setActive("preview");
    } finally {
...
export default function HomePage() {
  ...
  return <AppFrame><MainNav active={active} setActive={setActive} />{active === "home" && <MarketingHome setActive={setActive} />}{active === "why" && <WhyPage />}{active === "assessment" && <AssessmentPage setActive={setActive} setProfile={setProfile} setCarePlan={setCarePlan} setSavedIds={setSavedIds} setCompleted={setCompleted} />}{active === "preview" && <PlanPreview setActive={setActive} carePlan={carePlan} />}{active === "account-overview" && <AccountOverview carePlan={carePlan} savedIds={savedIds} completed={completed} toggleStep={toggleStep} setActive={setActive} />}{active === "account-providers" && <AccountProviders carePlan={carePlan} savedIds={savedIds} toggleSave={toggleSave} />}{active === "account-costs" && <AccountCosts carePlan={carePlan} />}{active === "account-family" && <AccountFamily carePlan={carePlan} />}</AppFrame>;
}
