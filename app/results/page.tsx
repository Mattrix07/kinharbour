    ...
    <main className="luxury-shell min-h-screen text-[#17231f]">
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-6 py-6 print:hidden">
        ...
        <div className="flex gap-3">
          <a href="/assessment" className="rounded-full border border-[#d8c9b7] bg-white/70 px-5 py-3 text-sm font-semibold text-[#173f35] shadow-sm backdrop-blur">Retake</a>
          <button onClick={() => window.print()} className="rounded-full bg-[#173f35] px-5 py-3 text-sm font-semibold text-white shadow-xl shadow-emerald-950/15">Print report</button>
        </div>
      </nav>
      ...
      <section className="mx-auto max-w-7xl px-6 pb-20 pt-6">
          ...
          <div className="grid gap-0 lg:grid-cols-[1fr_0.38fr]">
            <article className="p-8 md:p-12">
              ...
              <div className="mt-10 grid gap-4 md:grid-cols-3">
                <Metric label="Care complexity" value={`${result.score}/20+`} helper="Relative support signal" />
                <Metric label="Location" value={answers.location || "Not provided"} helper="Preferred search area" />
                <Metric label="Decision for" value={answers.relationship || "Family member"} helper="Family context" />
              </div>
            </article>
        ...
        <section className="mt-6 grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
          <Panel eyebrow="Risk view" title="What needs attention">
            <Checklist items={result.riskFlags} variant="risk" />
          </Panel>
          <Panel eyebrow="Action plan" title="Recommended next steps">
            <Timeline items={result.nextSteps} />
          </Panel>
        </section>

        <section className="mt-6 grid gap-6 lg:grid-cols-[0.82fr_1.18fr]">
          <Panel eyebrow="Preparation" title="Documents to gather">
            <Checklist items={result.documents} />
          </Panel>
          <Panel eyebrow="Provider search" title="Filters to apply">
        ...
        <section className="premium-card mt-6 rounded-[2.4rem] p-8 md:p-10">
          ...
          <div className="mt-8 grid gap-5 lg:grid-cols-3">
            {providers.map((provider) => (
              <article key={provider.name} className="group overflow-hidden rounded-[2rem] border border-[#eadfce] bg-white/80 shadow-sm transition hover:-translate-y-1 hover:shadow-2xl hover:shadow-stone-900/10">
                <div className="soft-highlight flex items-center justify-between border-b border-[#eadfce] p-5">
        ...
        <section className="mt-6 grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          <Panel eyebrow="Family discussion" title="Questions to align everyone">
  ...
  return (
    <ol className="relative grid gap-4">
      {items.map((item, index) => (
        <li key={item} className="flex gap-4 rounded-2xl bg-[#f7f1e8] p-4 leading-7 text-[#43544c] shadow-inner">
          <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#173f35] text-sm font-bold text-white">
            {index + 1}
          </span>
          <span>{item}</span>
        </li>
